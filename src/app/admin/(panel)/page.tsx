    ...
    <>
        ...
        >
          <span className="font-medium">
            {data.unreadMessages > 0
              ? `${data.unreadMessages} nouveau${data.unreadMessages > 1 ? "x" : ""} rendez-vous à traiter`
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {shortcuts.map(function (item) {
          ...
          const showNotif = item.href === "/admin/messages" && data.unreadMessages > 0;
          return (
            <Link
              key={item.href}
              href={item.href}
              className="group relative flex items-start gap-4 rounded-2xl border border-or/15 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md dark:bg-[#2a1a1d]"
            >
              {showNotif && (
                <span className="absolute right-4 top-4 rounded-full bg-rose-sombre px-2 py-0.5 text-xs font-medium text-white">
                  {data.unreadMessages} nouveau{data.unreadMessages > 1 ? "x" : ""}
                </span>
              )}
              <span
