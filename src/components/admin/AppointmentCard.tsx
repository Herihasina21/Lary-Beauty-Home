...
import { markAppointmentReadAction, updateAppointmentAction } from "@/app/admin/actions";
...
export function AppointmentCard({ appointment }: { appointment: Appointment }) {
  ...
  function markRead() {
    startTransition(async function () {
      await markAppointmentReadAction(appointment.id);
      router.refresh();
    });
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-or/10 bg-or/5 px-5 py-4">
        ...
        {!appointment.read && (
          <button
            ...
            onClick={markRead}
            disabled={pending}
            className="rounded-full bg-or px-3 py-1 text-xs font-medium text-white hover:bg-or/90 disabled:opacity-60"
          >
